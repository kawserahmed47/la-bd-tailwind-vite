            ...
            <div class="flex flex-col gap-9">
                ...
                <div class="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark px-3 pb-2.5 sm:px-5.5 xl:pb-1">
                    <div class="flex flex-col border-b border-[#eee] sm:flex-row sm:items-center sm:justify-between gap-3  py-4 px-4 md:px-6 xl:px-7.5">
                        <h2 class="text-bold text-lg font-weight-bolder">New project form</h2>
                        ...
                        <form id="project-store-form" method="POST" enctype="multipart/form-data" action="{{route('admin.project.store')}}">
                            @csrf
                            <div class="relative">
                                <div class="p-6.5">
                                            ...
                                            <div class="mb-6">
                                                ...
                                                <select id="status" name="status" required class="relative z-20 w-full appearance-none rounded border border-stroke bg-transparent py-3 px-5 outline-none transition focus:border-primary active:border-primary dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary">
                                                    @foreach (projectStatus() as $key=>$status)
                                                        <option value="{{$key}}" data-color="{{$status['color']}}" >{{$status['name']}}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                    ...
                                    <div class="mb-6">
                                        ...
                                        <textarea rows="2" id="description" name="description" placeholder="Type project description"
...
<script type="module">
    ...
    $(document).on('change', '#division_id', function(e){
        e.preventDefault();
        let _this = $(this);
        let _this_district = $('#district_id');

        $.ajax({
            type: "get",
            url: "{{route('division.options')}}",
            data: {
                "id" : _this.val()
            },
            beforeSend: function () {
            ...
            success: function (response) {
                ...
                _this_district.html(response.district_options)
                Toast.fire({icon: 'success', text: response.message});
            }, 
            error: function(xhr){
                ...
                _this.closest('form').find('.form-loaded').removeClass('hidden flex').addClass('hidden');
                _this_district.html(responseText.district_options)
            }
        });
    })
</script>
