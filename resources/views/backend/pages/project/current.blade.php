@extends('backend.master', ['title' => 'Project List', 'page' => 'project-current'])
...
@section('content')
    <div class="mx-auto max-w-screen-2xl p-4 md:p-3 2xl:p-5">
        @include('backend.partials.breadcrumb', [
            'breadcrumb_heading' => 'Project List',
        
            'breadcrumb_title' => 'Project',
            'breadcrumb_title_url' => route('admin.project.current'),
        
            'breadcrumb_subtitle' => 'List',
            'breadcrumb_subtitle_url' => route('admin.project.current'),
        ])
        ...
        <div class="flex flex-col gap-10">
            ...
            <div
                ...
                <div
                    ...
                    <h2 class="text-bold text-lg font-weight-bolder">Current project list</h2>
                ...
                <div class="relative max-w-full overflow-x-auto pb-5 project-current-table-section">
                    <table class="w-full table-auto">
                        ...
                        <tbody>
                                ...
                                @foreach ($projects as $project)
                                    <tr>
                                        <td class="border-b border-[#eee] py-3 px-4 pl-9 dark:border-strokedark xl:pl-11">
                                            <h5 class="font-medium text-black dark:text-white">{{ $project->name }}</h5>
                                            <p class="text-sm">{{ $project->bn_name }}</p>
                                        </td>
                                        <td class="border-b border-[#eee] py-3 px-4 dark:border-strokedark">
                                            <p class="text-black dark:text-white">{{ date('F d, Y', strtotime($project->updated_at)) }} </p>
                                        </td>
                                        <td class="border-b border-[#eee] py-3 px-4 dark:border-strokedark">
                                            <p
                                                class="inline-flex rounded-full  {{ projectStatus()[$project->status]['color'] }}  bg-opacity-10 py-1 px-3 text-sm font-medium">
                                                {{ projectStatus()[$project->status]['name']  ?? 'Draft' }}
                                            </p>
                                        </td>
                                        <td class="border-b border-[#eee] py-3 px-4 dark:border-strokedark">
                                            <div class="flex items-center space-x-3.5">
                                                ...
                                                <a data-tooltip-target="tooltip-download" data-tooltip-placement="top"
                                                    href="{{ route('admin.project.download', $project->slug) }}"
                                                    target="_blank"
                                                    class="hover:text-primary">
                                                    <svg class="fill-current text-gray-500 dark:text-white" width="18"
                                                        height="18" viewBox="0 0 18 18" fill="none"
                                                ...
                                                <form class="project-current-delete-form" method="POST"
                            ...
                            @else
                                <tr>
                                    <td colspan="4" class="text-danger text-center">No current projects found!</td>
                                </tr>
                            @endif
...
@push('js')
    <script type="module">
    $(document).on('submit', '.project-current-delete-form', function(e){
        e.preventDefault()
        let _this = $(this);
        let formLoaded = _this.closest('.project-current-table-section').find('.form-loaded');
