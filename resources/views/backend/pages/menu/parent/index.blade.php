        ...
        <div class="flex flex-col gap-10">
            <div class="rounded-sm border border-stroke bg-white px-3  pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-5.5 xl:pb-1">
                <div class="flex flex-row">
                    <div class="w-1/4">
                        @include('backend.pages.menu.tabs', ['activeTab' => 'parent'])
                    </div>
                    <div class="w-3/4 p-4">
                                        ...
                                        <div class="menu-label-accordion-row">
                                            <h2 id="menu-label-accordion-collapse-heading-{{$menu_label->id}}">
                                                <button type="button"
                                                    ...
                                                    <span>Menu Type: {{$menu_label->name}}</span>
                                                    ...
                                                    <div id="menu-label-accordion-collapse-{{$menu_label->id}}" data-accordion="collapse">
                                                        @if (count($menu_label->menus))
                                                                @foreach ($menu_label->menus as $menu)
                                                                    @include('backend.pages.menu.parent.partials.accordion', [
                                                                        'duplicate' => '',
                                                                        'form_action' => route('admin.menu.update', $menu->id),
    ...
    <script type="module">
        $(document).on('click', '.updateMenuForm .remove', function(e){
            ...
            if (result.isConfirmed) {
                ...
                success: function (response) {
                    _this.closest('.menu-label-parent-accordion-row').remove();
        ...
        $(document).on('click', '.updateMenuForm .clone', function(e){
            ...
            $.ajax({
                ...
                success: function (response) {
                    Toast.fire({icon: 'success', text: response.message});
                    $('#menu-label-accordion-collapse-'+_this.attr('data-label')).append(response.html);
                    formLoaded.removeClass('hidden flex').addClass('hidden');
                    initAccordions();
                },
                error: function(xhr) {
